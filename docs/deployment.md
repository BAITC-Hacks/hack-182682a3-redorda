...
`source_dir` и время импорта существующего Dataset, делая его текущим и сохраняя ID
и связанные запуски. Отсутствующие профили абонентов заполняются повторным импортом.
...
## Импорт CSV и хранение данных

Четыре CSV для демоимпорта входят в `data/demo/`. Пользовательские наборы сохраняются
в `backend/media/datasets/`; API нужен доступ на запись, worker — на чтение.
В Compose используется общий том `dataset_media`. Каталог не публикуется proxy.
Резервируйте его вместе с БД: встроенный `backups/` сохраняет только базу.

Миграции создают таблицу профилей абонентов. Повторный CLI-импорт полного пакета
заполняет её для существующего набора и снова делает его текущим без потери запусков.
Четыре сырых CSV содержат историю, но не готовый `customer_profile.csv`: просмотр и
создание черновика доступны, новый расчёт блокируется с объяснением. Это относится
к обеим встроенным factory. Старые планы полного пакета сохраняют свой dataset.

Альтернативная factory `apps.campaigns.services.public_environment.local_simulation`
использует код из `PARTICIPANT_KIT_DIR` с проверкой SHA-256 и данные выбранного набора.
Обе factory используют учебный публичный симулятор, не скрытый оценщик.

Импорт в браузере ожидает до пяти минут; Gunicorn и nginx дают 360 секунд.
Секреты и `.env.production` в Git не входят, текущая конфигурация сервера не меняется
при `git pull`. Пустую factory или выключенный флаг нужно обновить явно.
