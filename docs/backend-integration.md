...
- PostgreSQL/Redis и Compose jobs в CI, обновлённые OpenAPI и auth-контракт.
...
- **Docker отсутствует** на машине: `docker compose up` и сборка контейнеров локально
  не выполнялись. CI содержит отдельную проверку запуска; локальная валидация YAML
  не заменяет работающие контейнеры. Результат удалённого CI фиксируется отдельно.
