...
`Run.execution_blocker` — причина недоступности расчёта для конкретного набора
или `null`. Четыре сырых CSV без профиля не запускаются в локальном симуляторе;
черновик сохраняется, очередь не вызывается. Ограничение не затрагивает старые
планы полного пакета или повтор уже принятого идемпотентного запроса.
