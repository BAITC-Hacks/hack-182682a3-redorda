...
Base URL: `/api/v1/`. JSON-запросы и ответы используют UTF-8. UUID передаются строками, время — ISO 8601, денежные суммы — десятичными строками. API запускает расчёт через очередь; длительная работа не выполняется в HTTP-запросе.
