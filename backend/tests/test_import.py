...
def test_import_is_idempotent_and_aggregates_actual_rows(tmp_path):
    ...
    profile = dataset.subscribers.get(id_number="1").profile
    assert {key: profile[key] for key in ["ID_NUMBER", "predicted_arpu", "current_tariff",
...
def test_import_preserves_additional_profile_columns(tmp_path):
    ...
    with profiles.open(newline="") as file:
        rows = list(csv.DictReader(file))
    rows[0].update(ARPU_current="3.1400", customer_note="Original source text")
    ...
    assert dataset.subscribers.get(id_number="1").profile["customer_note"] == "Original source text"
...
def test_reimport_after_relocation_updates_existing_dataset_path_and_keeps_runs(
    ...
    assert dataset.imported_at > imported_at  # Reimport also makes this dataset current.
