...
function Detail({ id, meta }: { id: string; meta: Meta }) {
  ...
  return <div className="run-detail">
    ...
    <section className="panel execution-panel" aria-label="Управление расчётом">
      ...
      {run.status === 'draft' && <button className="button primary" disabled={!meta.features.run_execution || Boolean(run.execution_blocker) || busy !== null || loading} onClick={() => void action('start')}><Play size={16} />{busy === 'start' ? 'Запускаем…' : 'Запустить расчёт'}</button>}
      ...
      {run.status === 'draft' && run.execution_blocker && <div className="notice">{run.execution_blocker}</div>}
