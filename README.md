...
**Живая команда:** движок публикует задачи пяти ролей, результаты и передачи
после реальных шагов расчёта. Четыре персонажа используют сохранённые события.
`explain` объясняет выбранную кампанию по snapshot; `compare` проверяет начальную
часть сохранённого портфеля при новых ограничениях, учитывая уже потраченные
ресурсы пилотов. Это ограниченный сценарий, а не полный поиск нового портфеля;
его границы описаны в [API-контракте](docs/api-contract.md).
Обе команды не проводят новых пилотов. Ограничение каналов для запуска нового
плана пока ожидает поддержки движка.
